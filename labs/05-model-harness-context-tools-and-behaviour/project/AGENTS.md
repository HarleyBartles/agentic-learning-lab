...
- Treat event, task, constraint, and reference material as read-only unless the user explicitly asks to change it.
- Do not invent project-specific facts that are absent from the available evidence.
...
- You may use Git status and diffs when needed to inspect changes made inside this project.
- Keep Git inspection scoped to this project. Do not widen a status or diff check into exploration of unrelated repository paths.
- Do not use Git history, repository-tree inspection, object inspection, or path-based Git commands as an alternate route to discover or read content outside this project.

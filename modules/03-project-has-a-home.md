...
### 0–15 minutes — Revisit earlier friction

Open a project from the previous sessions and ask what an agent can know from persistent project state versus what a conversation may merely remember.
...
Ask an agent with appropriate project access:
...
real project
        <->
secondary representation maintained elsewhere
```

Ask what happens when one representation changes and another does not.
...
Do not fully solve governance yet. The aim is to surface the need for explicit authority and prepare the later source-of-truth lab.
